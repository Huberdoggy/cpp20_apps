...
void orderNumericInput(int numArr[], int size) {
	...
	{
		i < 2 ? printf("%d%s", numArr[i], ", ") : printf("%d", numArr[i]); 
        // I like using format strings where possible. Last index won't trail comma
	}
}
...
double getCoinValue(int length, double inputArr[]) {
	...
	for (int i = 0; i < length; i++) {
		sum = sum + (inputArr[i] * calcVals[i]); 
        // each iteration increment sum by multiplying the corresponding
        //  currency unit value by the num of that piece of currency passed in
	}
	return sum;
}
...
int main()
{
	...
	double inputArr[6] = { pennies, nickels, dimes, quarters, 
        halfDollars, dollars }; 
    // Not concerned with potential data loss here...need a double return
	int length = (sizeof(inputArr) / sizeof(inputArr[0])); 
    // For ease of use to track i loop in above function
	...
	cout << "\nYou have " << halfDollars << 
        (halfDollars != 1 ? " half dollars" : " half dollar.");
	cout << "\nYou have " << dollars << (
        dollars != 1 ? " dollars" : " dollar.");
	cout << "\n\nThe value of all your currency is: " << "$" 
        << getCoinValue(length, inputArr);

	cout << 
        "\n\nEnter 3 numbers to be sorted from smallest to largest, separated by spaces: "
        ; cin >> num1 >> num2 >> num3;
	int sortArr[3]{ num1, num2, num3 };
	int s = (sizeof(sortArr) / sizeof(sortArr[0])); 
    // pass this to function to track with the loop and use by 'sort' calculation
	cout << '\n';
